...
export default function App() {
  ...
  const generateImage = async () => {
    ...
    try {
      // Estrutura limpa e exata que o Flux 2 Pro exige
      const input = {
        prompt: prompt,
        aspect_ratio: aspectRatio,
      };

      // O SEGREDO ESTÁ AQUI: Enviando como um Array [ ]
      if (inputImage) {
 ...
 const downloadImage = async () => {
    ...
    try {
      // 1. Busca a imagem do servidor de forma "invisível" para não mudar de aba
      const response = await fetch(generatedImage);
      const blob = await response.blob();
      
      // 2. Transforma o arquivo em um objeto de imagem local
      const img = new Image();
      const objectUrl = URL.createObjectURL(blob);
      
      img.onload = () => {
        // 3. Cria um Canvas (quadro de pintura invisível) para converter o formato
        ...
        // 4. Extrai a imagem como PNG verdadeiro e força o download
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-gray-300 font-sans flex flex-col lg:flex-row">
      
      {/* PAINEL ESQUERDO LIMPO */}
      <div className="w-full lg:w-[400px] xl:w-[450px] bg-[#121214] border-r border-gray-800 p-6 overflow-y-auto max-h-screen custom-scrollbar">
        <h2 className="text-xl font-bold text-white mb-6">HUB 54 - ImAgem 1.0</h2>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-[#1c1c1f] border border-gray-700 rounded-md p-3 text-white focus:outline-none focus:border-white transition-colors resize-none h-24"
              placeholder="Descreva o cenário e a imagem..."
            />
          </div>
          ...
          <div>
            ...
            <div 
              ...
              className="w-full border border-dashed border-gray-700 hover:border-gray-500 rounded-md p-4 text-center cursor-pointer transition-colors bg-[#1c1c1f]"
            >
              <p className="text-sm text-gray-400">📄 Adicionar arquivo base</p>
              {inputImage && <p className="text-xs text-green-400 mt-2">✅ Imagem carregada com sucesso</p>}
            </div>
          ...
          <div>
            ...
            <select 
              ...
              className="w-full bg-[#1c1c1f] border border-gray-700 rounded-md p-2 text-white focus:outline-none"
            >
